...
- The CLI requires an immutable capture-registry manifest whose runner and transport entries each declare a UTF-8-path-sorted complete local ESM closure of exact `{relative_path,raw_sha256}` rows. The exact-pinned `es-module-lexer` discovers static imports, re-exports, and literal dynamic imports recursively; the registry rejects missing/extra/duplicate rows, non-literal dynamic imports, unapproved bare or external imports, root/realpath escape, symlinks, non-files, cycles, and any byte change before import, after import, or immediately before CLI target/CAS use. Only the transport closure may statically import the fixed `node:http` allowlist entry. Closure identities are bound into the registry and capture-environment digest together with the exact case/task-script digest. The runner must activate every required `(replay_profile_id,task_step_id)` in task order; only requests and observations made while that step is active may receive its label. The CLI never accepts caller-self-reported implementation digests or infers actions from natural-language `instruction` text.
