...
- The CLI requires an immutable capture-registry manifest whose runner and transport entries each declare a UTF-8-path-sorted complete local ESM closure of exact `{relative_path,raw_sha256}` rows. The exact-pinned `es-module-lexer` discovers static imports, re-exports, and literal dynamic imports recursively; the registry rejects missing/extra/duplicate rows, non-literal dynamic imports, every builtin/bare/external import, root/realpath escape, symlinks, non-files, cycles, and any byte change before loading or immediately before CLI target/CAS use. Registry state retains only verified immutable closure buffers and identities: construction uses an isolated validation stage and immediately destroys it, so unused entries own no persistent stage. Every accepted CLI invocation independently copies its selected runner and transport buffers into fresh private read-only content-addressed staging roots; entry, static/re-export helpers, and delayed literal-dynamic helpers execute only from those invocation-owned roots, never from caller-visible source URLs or their ambient Node ESM cache. Source and staging bytes are reverified before target/CAS access, the per-run roots remain alive through runner/transport completion, and that invocation alone destroys them in `finally` on success, partial, or throw. Closure identities are bound into the registry and capture-environment digest together with the exact case/task-script digest. The runner must activate every required `(replay_profile_id,task_step_id)` in task order; only requests and observations made while that step is active may receive its label. The CLI never accepts caller-self-reported implementation digests or infers actions from natural-language `instruction` text.
...
- Create: `evaluator/manifest.json` with exact evaluator file digests, the repo-pinned `evaluator/snapshot-source-registry.json` raw digest, and the finalized schema/knowledge/policy manifest digests
...
```bash
git add evaluator/index.mjs evaluator/projection.mjs evaluator/manifest.json evaluator/snapshot-source-registry.json schemas/evaluator/output.schema.json schemas/evaluator/semantic-projection.schema.json schemas/manifest.json evals/tests/evaluator.test.mjs evals/tests/evaluator-task9-provenance.test.mjs evals/golden/high-risk-delete.json
...
The implementation must recompute source_material_digest from exact validated normalized source objects, never trusts a caller-supplied source_material_digest, and uses these exact preimages: Claim `{claim,evidence,policy}`; Risk `{finding_id,finding,context}`; Recommendation `{parts,selected_policy_registry_row}`; Release `{derived_gates,selection,authority,critical_tail_evidence,conditions}`. Canonical set permutation must produce byte-identical public output. Risk `finding` is the complete Task 5 Finding: `finding_id` must equal `finding.finding_id`, match `^f_[0-9a-f]{32}$`, and resolve exactly once in the same `EvaluationOutput.findings`. The materializer recomputes the Task 5 fingerprint full digest and id, byte-compares the resolved owner with the embedded Finding, and re-derives the five-field risk summary; missing, dangling, mismatched, duplicate, or incoherent owners are invalid.
...
Task 10 tests tamper policy/check/evidence/dimension material for Claim, purpose/material-reliance and complete Finding ownership for Risk, authority/evidence/risk material for Recommendation, and gates/selection/authority/critical-tail material for Release. Two distinct valid Findings with the same five-field risk summary must produce distinct `source_material_digest` and `risk_assessment_id` values. Every material change must change both source_material_digest and the public id; set permutation must not.

Task 10 public-shape acceptance requires EvaluationOutput and SemanticProjection to reference deep-equal full Task 5 producer definitions: ten-field RuleEvaluation, seven-field Finding, and unchanged closed three-field RunIssue. Genuine evaluateRule/emitFinding results plus their RiskAssessment owner must validate in both roots, while legacy four-field and three-field projections must fail.
