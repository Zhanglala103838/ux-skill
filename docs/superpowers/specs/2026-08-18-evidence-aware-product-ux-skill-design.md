...
| evaluator / ux-skill:evaluator:v1 | v0.1 verified-registry public identity is frozen at `51a663d748bf762d3f52a64b0b1a553c68c47b15d556c0c7ba80da3f923207ba` after artifact verification；registry-unavailable identity is `digestJcs("ux-skill:evaluator-registry-unavailable:v1", "SNAPSHOT_SOURCE_REGISTRY_UNAVAILABLE")` and cannot consume registry raw or binding values |
...
~~~json
{"adapter_contract_id":"hulianui.get-component-doc.alert-dialog.v2","component_identity":{"category":"feedback","name":"AlertDialog","slug":"alert-dialog"},"evidence_scope":["component-identity","import","exports","props","events","slots"],"npm_integrity":null,"prohibited_claims":["ux-outcome","wcag-conformance","user-success","complete-destructive-flow"],"provider_namespace":"https://hulianui.haloritual.com/mcp","repository_commit":"c0edaf47c4e889abd637aa07875da907666c4aa1","request":{"format":"json","name":"alert-dialog","sections":["props","events","slots"]},"server_package":"@hulianui/mcp","server_version":"0.11.0","source_artifact":{"path":"llms-props.json","sha256":"9b7022c6a66c72340469844e4fba1e7e449d5474fb2d4427e0a49be7783da293","version":"0.57.0"},"tool_name":"get_component_doc"}
~~~

adapter_contract_digest（JCS SHA-256）固定为 a14b5fec69cebe5802d6d34ff02fd8e4b571b3468832127b6a43c19989e0b4a9。
...
| ADAPTER-HULIAN-ALERT-001 | 固定 HulianUI JCS row digest=a14b5fec69cebe5802d6d34ff02fd8e4b571b3468832127b6a43c19989e0b4a9；request/result 只形成 component/import/exports/props/events/slots evidence，不形成 prohibited claims |
...
CLI 不解释自然语言 `instruction`，也不得把入口页或同一 DOM 复制后贴到多个 `task_step_id`。它只接受 immutable capture-registry manifest：registry 固定 case ID、exact task-script digest，并为 runner/transport 分别登记 UTF-8 path 排序的完整本地 ESM closure `{relative_path,raw_sha256}`。runtime 使用 exact-pinned `es-module-lexer` 递归解析 static import、re-export 和 literal dynamic import；missing/extra/duplicate、non-literal dynamic import、任何 builtin/bare/external import、root/realpath escape、symlink、non-file、cycle 或任何 raw-byte 变化均 fail closed。registry 私有状态只保留 verified immutable closure buffers/identities；构造期的 isolated validation stage 在校验 exports 后立即销毁，未选择 entry 不占有 stage。每次 CLI 调用都从所选 buffers 新建该 invocation 独占、只读、content-addressed 的 runner/transport staging roots；entry、static/re-export helper 与延迟 literal-dynamic helper 只从本次隔离 root 执行，不读取 caller-visible source URL，也不信任 ambient Node ESM cache。runtime 在 staging import 前后及 CLI 触达 target/CAS 前重验 source 与 staged closure；每组 roots 的生命周期覆盖本次 runner/transport 完整调用，并只由该 invocation 的 `finally` 在 success、partial 或 throw 后销毁，因此并发 invocation 互不删除。closure identities 同时进入 registry digest 与 capture-environment digest，并以不可伪造的 registry capability 选择实现。caller 自报 function/digest 不构成注册。runner 必须按每个 profile 的 task 顺序显式激活每个 step；network 与 observation 的 `task_step_id` 只能由当前 active step 写入。runner 只编排 request 与 consume-observation，不能提交 observation bytes；可信 transport/browser owner 返回证据 bytes，runtime 将其转成与当前 active step 绑定、single-use、跨 step 不可复用的 opaque handle。transport closed response 还必须给出完整 ordered redirect_chain；每个 hop 固定连续 sequence、status、url、解析后的 location 与原始 ordered response headers，Location 必须解析到该 location，所有 hop header bytes 进入同一 CAS read-back/digest 路径。每个 active step 至少产生一项 GET/HEAD network record 和一项真实 observation；缺链/乱序/篡改、跨 step 复用、漏步、重复、未知 step、registry/runner/transport 缺失或任何 digest/task/closure 变化一律 `incomplete + target_unavailable + no_release + exit 2`，且不得触达未注册目标。
...
Task 10 对 `black_box_site` 的 SnapshotClosure 只接受仓库固定的 `evaluator/snapshot-source-registry.json` 作为来源权威。该闭合注册表逐行绑定 source authority id、target snapshot id/kind、canonical locator、entry URL、task script digest、capture environment digest（包含 producer runner/transport closure identity）及允许的 closure/snapshot digest；调用方 bundle 不提供、不能新增或覆盖注册行。evaluator 必须在完成 closure bytes、CAS 与 replay 重算后再与唯一注册行逐字段精确匹配；缺失、未注册、coordinated relabel、digest 不在 allowlist 或注册表/manifest tamper 一律 fail closed 为 `/snapshot_closure` critical coverage gap 与 `no_release`。注册表 JSON 由 `evaluator/manifest.json` 的 raw-byte digest 认证，但不计入固定九个 `.mjs` import closure；认证成功时保留冻结的 v0.1 public evaluator identity，注册表 raw/binding 不可用时只使用固定 domain-separated sentinel identity，绝不把未验证值带入 semantic projection。 `canonical_locator` 与 `entry_url` 是两个独立身份字段：前者必须使用 `canonical-relative-path` 的稳定仓库标识，后者必须是实际绝对 HTTP(S) 入口；四个 public authority row 固定为 `public-sites/rw-docs-stripe-001`、`public-sites/rw-website-apple-001`、`public-sites/rw-website-govuk-001`、`public-sites/rw-website-ikea-001`，并分别绑定原绝对入口 URL。所有注册行在匹配前先使用与 EvaluationInputBundle 相同的路径语法自校验。仅 authority registry 自身的 raw bytes 或 manifest binding 不可验证时，evaluator 不读取或使用该内容，而是从 bundle、固定 snapshot-unavailable reason table 与固定 unavailable evaluator sentinel 生成非权威 `/snapshot_closure` critical gap 和 `no_release` 结果；raw/binding 两种失败必须得到 byte-equal semantic projection、semantic digest 与 evaluator identity，audit sidecar 的闭合状态固定为 `manifest_verification: "partial_failure"`、`snapshot_source_registry: "unavailable"`。schema、knowledge、policy 或九模块 evaluator artifact 的 tamper 仍抛 `ARTIFACT_VERIFICATION_FAILED`。
...
| RW-HULIAN-DELETE-001 / HulianUI 迁移目标 / fixed_anchor / admin-internal-tool | 第 13 节 adapter_contract_id=hulianui.get-component-doc.alert-dialog.v2；row digest=a14b5fec69cebe5802d6d34ff02fd8e4b571b3468832127b6a43c19989e0b4a9 | 把一个高风险 Admin 删除审批场景映射为 AlertDialog candidate，保留标题、说明、确认/取消、busy/error/retry、keyboard/focus requirements | 组件 contract 只能证明候选能力，完整安全体验取决于产品流、状态与运行验证 | component contract + 后续隔离 harness；不得用组件存在推导 flow complete 或 WCAG conformant |
...
Task 6 reducers return only the closed internal contracts `ClaimAssessmentCoreV1`, `RiskDecisionCoreV1`, `RecommendationDecisionCoreV1`, and `ReleaseDecisionCoreV1`. Task 10 is the sole public materializer; no Task 6 helper emits an authority-bearing public assessment.

Task 10 recomputes source_material_digest from exact validated normalized source objects and never trusts a caller-supplied source_material_digest. The exact source preimages are: Claim `{claim,evidence,policy}`; Risk `{finding_id,finding,context}`; Recommendation `{parts,selected_policy_registry_row}`; Release `{derived_gates,selection,authority,critical_tail_evidence,conditions}`. Every set-valued member is NFC/I-JSON validated, duplicate-conflict checked, and sorted by its declared canonical key before JCS; sequence fields retain their declared order. `source_material_digest = SHA-256(JCS(normalized_source))`.

- ClaimAssessment preserves `policy_id`, every full ClaimCheckResult in `checks`, every full EvidenceDimensionScore in `dimension_scores`, the structured EvidenceGrade, the selected-branch predicate, and reason codes. It never replaces checks with `required_check_id + ":" + status`, and the former lossy `validity,directness,precision,transportability` integer projection is forbidden. `claim_assessment_id = "ca_" + first32hex(SHA-256("ux-skill:claim-assessment-public:v1" || JCS(complete_id_less_public_body)))`.
- RiskAssessment source `finding` is the complete Task 5 Finding, and `finding_id` must equal `finding.finding_id`, match `^f_[0-9a-f]{32}$`, and resolve exactly once in the same `EvaluationOutput.findings`. Task 10 recomputes the Task 5 fingerprint full digest and `finding_id`, byte-compares the resolved owner with the embedded Finding, and re-derives the five-field risk summary from that owner; missing, dangling, mismatched, duplicate, or incoherent owners are invalid. `source_material_digest` hashes the full canonical `{finding_id,finding,context}` preimage, and `risk_assessment_id` uses `ux-skill:risk-assessment:v1` over the complete id-less public body, so distinct valid Findings remain distinct even when their risk summaries are equal.
- RecommendationAssessment preserves all four ceilings plus `minimum_ceiling`, its exclusive recommendation/research-question branch, action, policy version, and reason codes. Nested Recommendation uses `ux-skill:recommendation:v1`; `recommendation_assessment_id` uses `ux-skill:recommendation-assessment:v1` over the complete id-less public body.
