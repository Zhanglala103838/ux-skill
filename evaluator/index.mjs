...
import{ARTIFACT_READ_LIMITS,canonicalSet,assertCanonicalRelativePath,assertIJson,jcsBytes,parseArtifactJson,readArtifactBytes}from'./canonical.mjs';
...
const artifactMaxBytes=(path)=>path.endsWith('.json')?ARTIFACT_READ_LIMITS.JSON_MAX_BYTES:ARTIFACT_READ_LIMITS.RAW_MAX_BYTES;
const loadRaw=async(path)=>{const bytes=await readArtifactBytes(new URL(path,ROOT),{maxBytes:ARTIFACT_READ_LIMITS.JSON_MAX_BYTES});const value=parseArtifactJson(bytes,path);try{assertIJson(value);}catch{artifactFailure('ARTIFACT_JSON_INVALID:'+path);}if(!nfc(value))artifactFailure('ARTIFACT_NFC_INVALID:'+path);return{bytes,value};};
const verifyRows=async(rows,label)=>{if(!Array.isArray(rows))artifactFailure(label+'_ROWS_INVALID');const sorted=canonicalSet(rows,(row)=>row.path);if(!jcsBytes(sorted).equals(jcsBytes(rows)))artifactFailure(label+'_ORDER_INVALID');for(const row of rows){if(!exact(row,['path','file_digest'])||!/^[-a-z0-9_./]+$/.test(row.path)||!/^[0-9a-f]{64}$/.test(row.file_digest))artifactFailure(label+'_ROW_INVALID');const bytes=await readArtifactBytes(new URL(row.path,ROOT),{maxBytes:artifactMaxBytes(row.path)});if(sha(bytes)!==row.file_digest)artifactFailure(label+'_DIGEST_MISMATCH:'+row.path);}};
