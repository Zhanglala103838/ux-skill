...
const brokerAccounted=(lease,returned)=>{
  try{
    if(!plain(returned)||!plain(lease.request)||!['GET','HEAD'].includes(lease.request.method)||!url(lease.request.url)||!Array.isArray(returned.redirect_chain)||!Array.isArray(returned.headers)||!(returned.body instanceof Uint8Array))return null;const admitted=lease.admitted;if(admitted.length!==returned.redirect_chain.length+1||admitted.some(record=>record.state!=='fulfilled'))return null;const redirects=[];let expectedUrl=lease.request.url;
    for(let index=0;index<returned.redirect_chain.length;index+=1){const record=admitted[index],hop=returned.redirect_chain[index];if(!plain(hop)||record.input.method!==lease.request.method||record.input.url!==expectedUrl||record.input.url!==hop.url||record.response.status!==hop.status||record.response.body.length!==0||!Array.isArray(hop.response_headers))return null;const brokerOwned=canonHeaders(snap(record.response.header_rows)),transportOwned=canonHeaders(snap(hop.response_headers));if(!jb(brokerOwned).equals(jb(transportOwned)))return null;const locations=brokerOwned.headers.filter(row=>row.name_lower_ascii==='location').map(row=>Buffer.from(row.value_bytes_base64,'base64').toString('latin1'));if(!locations.some(location=>{try{return new URL(location,hop.url).href===hop.location}catch{return false}}))return null;redirects.push({...hop,response_headers:brokerOwned.headers});expectedUrl=hop.location}
    const final=admitted.at(-1),brokerOwned=canonHeaders(snap(final.response.header_rows)),transportOwned=canonHeaders(snap(returned.headers));if(final.input.method!==lease.request.method||final.input.url!==expectedUrl||final.input.url!==returned.final_url||final.response.status!==returned.status||lease.request.method==='HEAD'&&(final.response.body.length!==0||returned.body.length!==0)||!Buffer.from(final.response.body).equals(Buffer.from(returned.body))||!jb(brokerOwned).equals(jb(transportOwned)))return null;return{...returned,request_method:final.input.method,request_url:admitted[0].input.url,headers:brokerOwned.headers,redirect_chain:redirects};
  }catch{return null}
};
  ...
  return{cas,async capture(c){
    ...
    const executeStep=async(identity,work)=>{
      ...
      const request=async(input)=>{
        ...
        const response=await registration.request(snap(value));if(!plain(response)||!exact(response,['status','final_url','request_method','request_url','headers','body','redirect_chain','observation_artifacts'])||!Number.isInteger(response.status)||response.status<100||response.status>599||!['GET','HEAD'].includes(response.request_method)||response.request_method!==value.method||!url(response.request_url)||response.request_url!==value.url||!url(response.final_url)||!Array.isArray(response.headers)||!Array.isArray(response.redirect_chain)||response.redirect_chain.length>L.maxRedirectHops||!Array.isArray(response.observation_artifacts)||!(response.body instanceof Uint8Array)||utilTypes.isProxy(response.body)||response.body.length>L.maxArtifactBytes||response.request_method==='HEAD'&&response.body.length!==0)throw E('TASK_RUNNER_RESPONSE_INVALID');
